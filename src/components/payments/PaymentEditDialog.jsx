                            ...
                            <Card className="bg-gradient-to-br from-slate-50 to-white border-slate-200 shadow-sm">
                                <CardContent className="p-4">
                                    <div className="grid grid-cols-3 gap-4">
                                        ...
                                        {/* Déjà Payé */}
                                        <div className="flex flex-col">
                                            <span className="text-slate-500 text-xs font-medium mb-1">Déjà Payé</span>
                                            <span className="text-lg font-bold text-green-600">
                                                {(totalPaid - parseFloat(payment.amount)).toLocaleString()} <span className="text-xs font-normal">Ar</span>
                                            </span>
                                        </div>

                                        {/* Reste à Payer */}
                                        <div className="flex flex-col">
                                            <span className="text-slate-500 text-xs font-medium mb-1">Reste à Payer</span>
                                            <span className="text-lg font-bold text-red-600">
                                                {remainingAmount.toLocaleString()} <span className="text-xs font-normal">Ar</span>
                                            </span>
                                        </div>
                                    </div>
                                    ...
                                    <div className="mt-3">
                                        <div className="w-full bg-slate-200 rounded-full h-2">
                                            <div
                                                ...
                                                style={{ width: `${totalAmount > 0 ? ((totalPaid - parseFloat(payment.amount)) / totalAmount) * 100 : 0}%` }}
                                            ></div>
                                        </div>
                                        <p className="text-xs text-slate-500 mt-1 text-center">
                                            {totalAmount > 0 ? Math.round(((totalPaid - parseFloat(payment.amount)) / totalAmount) * 100) : 0}% payé
                                        </p>
                                    </div>
                                </CardContent>
                            </Card>
                        ...
                        {remainingAmount > 0 && (
                            <Card className="bg-gradient-to-br from-slate-50 to-white border-slate-200 shadow-sm">
                                <CardContent className="p-4 flex justify-between items-center">
                                    <div className="flex flex-col">
                                        <span className="text-slate-500 text-sm font-medium">Reste à payer sur la facture</span>
                                        <span className="text-2xl font-bold text-slate-800 tracking-tight">
                                            {remainingAmount.toLocaleString()} <span className="text-sm font-normal text-slate-500">Ar</span>
                                        </span>
                                    </div>
