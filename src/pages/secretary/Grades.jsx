...
export default function Grades() {
  ...
  const { data: gradesData, isLoading } = useQuery({
    queryKey: ["grades"],
    queryFn: () => gradeService.getAllGrades({ limit: 1000 }),
  });
  ...
  const getGradeDistribution = () => {
    const ranges = [
      ...
      { range: "5-10", count: 0, color: "#F43F5E" },
      { range: "10-15", count: 0, color: "#EC4899" },
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      <div className={`w-12 h-12 rounded-full ${index === 0 ? "bg-green-500" :
                        index === 1 ? "bg-pink-400" :
                          index === 2 ? "bg-rose-500" :
                            "bg-red-600"
